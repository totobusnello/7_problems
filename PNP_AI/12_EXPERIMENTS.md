...
- **Resultado 4 (FECHO parcial, 2026-07-14): lote 3-out auditado 102.655/606.933 (16,9%) na VPS — 99.067 ótimas + 3.370 SUBÓTIMAS certificadas + 218 inconclusivas** (cap SAT 30 s contra a cauda k-alto UNSAT-hard). **Total do Alvo C: 109.909 auditados, 106.271 ótimas, 3.420 melhorias certificadas** (2-out 50 + 3-out 3.370); as 3.370 melhorias 3-out re-verificadas por simulação local (3.183× −1 AND, 115× −2, 72× −3). Fecho de ENGENHARIA (retorno decrescente; cobertura parcial DECLARADA, não exaustão). PR #8 squash-merged (databases 121 MB fora do histórico via `.gitignore` + `fetch_databases.sh` por sha256). Nota: `notes/audit_alvoC_simplifier_findings.md`.
- **Aberto:** (a) restante do lote 3-out (83%) parqueado (ROI baixo); (b) base BENCH sob o modelo de custo deles; (c) revisão adversarial (2 famílias) antes de qualquer contato com os autores — que exige OK de Luiz.

## EXP-XAG-N5 — Busca amostral n=5 por separador do Unit Gap em XAG (2026-07-15, claim 0030)

- **Pergunta:** Open Question #1 (`14_OPEN_QUESTIONS` / `notes/technote_unitgap_basedep` §7) — `gap_XAG` permanece em {0,1} em n=5, ou aparece um separador não-linear?
- **Método:** candidatos gerados por circuitos XAG aleatórios (viés de sharing + XOR ⟹ opt baixo, estrutura não-linear — onde um separador é mais provável), filtro essencial-nas-5, `opt_XAG` e `tree_XAG` por SAT (busca ascendente, cap 20 s). Encoder de FORMULA (fan-out 1, sem dedup) validado ANTES por gate G-T: **G-T1** opt intacto 222/222; **G-T2** n=3 256/256 `tree=opt`; **G-T3** n=4 222/222 `tree_via_sat = tree_DP` (DP independente), cobrindo as 4 classes com gap=1. Seeds determinísticos por worker (`Random(90210+wid)`); 2 workers; VPS 2 vCPU.
- **Falso positivo corrigido antes da coleta:** parity-5 (`0x69969669` etc — linear, gap 0) reportado como separador na 1ª versão (UNSAT em opt+1 isolado ≠ `tree ≥ opt+2`); corrigido p/ busca ascendente a partir de opt (o 1º k SAT é `tree`). Parity-5 reconfirmado gap 0.
- **Resultado:** **29.643 funções de 5 vars — gap0 29.143 (98,31%), gap1 296 (1,00%), inconclusive 204 (0,69%, timeout SAT), separadores (gap ≥ 2): 0.** AMOSTRAL, não exaustivo (2³² tabelas inviabilizam exaustão) — evidência a favor de `gap_XAG ∈ {0,1}` em n=5; rebaixa a probabilidade de um separador de opt baixo. **Claim 0030.**
- **Artefatos:** `experiments/exp_xag_n4/search_n5.py`, `gate_xag_tree.py`, `search_n5_w0{0,1}.csv`; `notes/xag_n5_search_findings.md`. Solver kissat 4.0.4. PR #9.
